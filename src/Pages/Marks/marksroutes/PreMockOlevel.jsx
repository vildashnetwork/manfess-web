...
export default function PreMockOlevel() {
 ...
 const [loading, setloading] = useState(false)
  const getresult = async ()=>{
    try{
      setloading(true)
 const res = await axios.get("https://manfess-backend.onrender.com/api/students/olevelpremock/all")
  ...
  const [load, setLoad] = useState(false)
   ...
   try {
        setLoad(true);
        await axios.get("https://manfess-backend.onrender.com/api/students/print/premockolevel/print-slips", {
          responseType: 'blob'
        }).then(response => {
  ...
  const [load1, setLoad1] = useState(false)
   ...
   try {
        setLoad1(true);
        await axios.get("https://manfess-backend.onrender.com/api/students/print/premockolevel", {
          responseType: 'blob'
        }).then(response => {
          // Create blob link to download
          const url = window.URL.createObjectURL(new Blob([response.data]));
